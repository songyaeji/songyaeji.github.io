...
승인 이후의 도구 정의 변조를 시계열 관점에서 감시·탐지하는 방법을 설계하고 구현하는
것이 목표이며, SIFT Lab(경희대)에서 진행 중인 연구라 구체적인 방법론·결과는 아직
비공개다. 공개 가능한 시점에 갱신한다.

선행 연구로 MCP 서버 메타데이터 기반 공급망 위협 탐지 연구(Smithery Marketplace
3,806개 서버 분석)를 [KCC 2026에서 발표](/publications/)했고, 연구 과정에서 수집한
MCP 생태계 데이터는 [MCP Dashboard](https://songyaeji.github.io/mcp-dashboard/)에서
확인할 수 있다.
